<template>
  <div class="mod-distribution-user">
    <avue-crud ref="crud" :page="page" :data="dataList" v-model="dataForm" :table-loading="dataListLoading" :option="tableOption"
      @search-change="searchChange" @selection-change="selectionChange" @search-reset="searchReset" @on-load="getDataList">
      ...
      <template slot-scope="scope" slot="qrCode" v-if="scope.row.distributionUser">
      ...
      <template slot="menuLeft">
        <el-button type="primary" icon="el-icon-plus" size="small" v-if="isAuth" @click.stop="addOrUpdateHandle()">新增分销员</el-button>
      </template>

      <template slot-scope="scope" slot="menu">

        <el-button type="danger" size="small" @click="leaveHandle(scope.row.distributionUserId)" v-if="isAuth">离职</el-button>

        <!-- <el-button type="primary" size="small" icon="el-icon-edit" v-if="isAuth" @click="transferStore(scope.row.distributionUserId,scope.row.storeName)">转店</el-button> -->
      </template>
    ...
    <transfer-store v-if="tranfersVisible" ref="transferStore" @refreshDataList="refreshChange"></transfer-store>
  </div>
</template>
<script>
  import {
    tableOption
  } from '@/crud/shop/distributionUser.js'
  import AddOrUpdate from './distribution-user-update'
  import TransferStore from './distribution-user-transfer-store'
  import ShowCode from '../distribution/user/show-staff-code.vue'
  export default {
    data() {
      return {
        dataForm: {
