...
Page({
  data: {
    ...
    groupcoverImg: '',
    times:'',
    // '&beginDate=' + beginDate + '&endDate=' + endDate + '&laterDays=' + laterDays,
    // validateType
  },
  ...
  onLoad: function (options) {
    ...
    if (options.validateType > 1){
      that.setData({
        times: options.beginDate + '～' + options.endDate
      })
    }else{
      that.setData({
        times: '领取后' + options.laterDays + '天有效'
      })
    }
    ...
    wx.getSystemInfo({
      success: function (res) {
        that.setData({
          ...
          normalPageX:
            res.windowWidth - (res.windowWidth + res.windowWidth * 0.77) / 2, //左边文本图片X轴位置
          ...
          imgHeight: res.windowWidth * 0.76, //商品图片高度
          imgPageY: res.windowWidth * 0.28, //商品图片Y轴位置
          codeWidth: res.windowWidth * 0.192, //小程序码图片宽度
          codeHeight: res.windowWidth * 0.192, //小程序码图片高度
          codePageY: res.windowWidth * 0.222 + res.windowWidth * 0.72 + 45, //小程序码Y轴位置
          avatarPageY: res.windowWidth * 0.222 + res.windowWidth * 0.72 + 15, //头像Y轴位置
          titlePageY: res.windowWidth * 0.222 + res.windowWidth * 0.72 + 66, //标题Y轴位置
          specPageY: res.windowWidth * 0.222 + res.windowWidth * 0.72 + 0, //规格Y轴位置
          timePageY: res.windowWidth * 0.222 + res.windowWidth * 0.72 + 90, //时间Y轴位置
          pricePageY: res.windowWidth * 0.222 + res.windowWidth * 0.72 + 118, //价格Y轴位置
  ...
  createNewImg: function (lineNum) {
    ...
    ctx.fillText(
      ...
      (that.data.codeWidth - 30) / 2,
      that.data.codePageY + that.data.codeWidth + 14
    );
    ctx.draw(); //绘制到canvas
  },
