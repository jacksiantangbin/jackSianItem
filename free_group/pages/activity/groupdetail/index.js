...
Page({
    ...
    util.request(api.groupDetailUrl, condition).then(function(res) {
      if (res.errno == 0) {
        ...
        if (res.data.group.itemDetail){
          WxParse.wxParse('article', 'html', res.data.group.itemDetail, that, 5);
          that.setData({
      ...
      if (endDate < 1000) {
        ...
        if (isRegister == 1) {
          ...
          util.request(api.offeredUrl, condition).then(function(res) {
            if (res.errno == 0) {
              wx.navigateTo({
                url: '../../../pages/pay/item4/itemPay?groupId=' + groupId
              })
            } else {
              wx.showToast({
                title: res.errmsg,
  ...
  groupList: function(e) {
    ...
    wx.navigateTo({
      url: '../../../pages/activity/groupList/index?groupId=' + groupId + '&orderId=' + orderId + '&personCount=' + personCount,
    })
  },
  telPhone: function(e) {
