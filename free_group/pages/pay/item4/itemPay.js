  ...
  groupOrderData: function() {
    ...
    util.request(api.groupOrderUrl, data).then(function(res) {
      ...
      that.setData({
        ...
        // storeList: res.storeList.slice(0, 1),
        shareGold: res.shareGold,
        // share_accountAmount: res.shareGold.accountAmount,
      });
      // }
  ...
  pay: function() {
    ...
    util.request(api.groupOrder, data).then(function(res) {
      if (res.errno == 0) {
        ...
        if (lastPrice == 0) {
          wx.navigateTo({
            url: '../../../pages/pay/result/payResult?orderCode=' + orderCode + '&lastPrice=' + lastPrice + '&title=' + title + '&payid=' + groupId + '&itemType=2' + '&orderId=' + orderId + '&activityState=' + activityState + '&activityNum=' + activityNum,
          })
        } else if (lastPrice > 0) {
          that.zooPay()
        }
    ...
    util.request(api.getPayParams, data).then(function(res) {
      if (res.errno == 0) {
        wx.requestPayment({
          ...
          fail(res) {
            wx.navigateTo({
              url: '../../../pages/pay/result2/payResult?orderCode=' + orderCode + '&lastPrice=' + lastPrice + '&title=' + title + '&payid=' + groupId + '&orderId=' + orderId + '&itemType=2' + '&activityNum=' + activityNum,
            })
          }
        })
      } else {
