    ...
    util.request(api.groupActivityDetailUrl, condition).then(function(res) {
      if (res.errno == 0) {
        ...
        that.setData({
          ...
          groupRule: res.data.detail.groupRule,
        ...
        if (res.data.steps) {
          ...
          var step_arry = []
          var step_arry2 = []
          var personCount = res.data.groupActivity.personCount + 1
          // var personCount = 3
          ...
          for (var j = 0; j < step_arry.length; j++) {
            if (step_arry[j] == "" || step_arry[j] == null || typeof (step_arry[j]) == "undefjned") {
              ...
              j = j - 1;
              that.setData({
                // step_price_pay: step_arry.slice(0, 1)
        ...
        if (that.data.itemDetail){
          WxParse.wxParse('itemDetail', 'html', that.data.itemDetail, that, 5);
        }
   
        if (that.data.groupRule) {
          WxParse.wxParse('groupRule', 'html', that.data.groupRule, that, 5);
        }
      ...
      if (endDate < 1000) {
        ...
        if (isRegister == 0) {
          ...
          util.request(api.offeredUrl, condition).then(function(res) {
            if (res.errno == 0) {
              wx.navigateTo({
                url: '../../../../pages/pay/item4/itemPay?groupId=' + groupId + '&groupActivityId=' + groupActivityId + '&activityState=' + activityState + '&isOffered=0',
              })
            } else {
              var userInfo = wx.getStorageSync('userInfo');
